<div class="min-h-screen bg-white dark:bg-gray-900 p-8">
    <div class="max-w-5xl mx-auto space-y-8">
        ...
        <div class="bg-gradient-to-r from-sky-700 to-sky-900 dark:from-blue-900 dark:to-blue-800 border-0 shadow-lg rounded-lg p-2">
            <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-6 p-2">
                <div class="space-y-2">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-blue-100">
                            <span class="font-semibold">Last Visit:</span> {{ $patient->last_visit_at ? $patient->last_visit_at : 'No visits yet' }}
                        </p>
                    </div>
                </div>
        ...
        <div>
            ...
            <div class="border border-gray-300 dark:border-gray-700 rounded-md p-2 shadow-md hover:shadow-lg transition-shadow">
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        ...
                        <flux:badge :variant="$latestCard ? 'warning' : 'success'" size="lg">
                            {{ $latestCard ? 'Pending' : 'Paid' }}
                        </flux:badge>
                    </div>

                    @if($latestCard)
                        <div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-6 space-y-6 border border-gray-200 dark:border-gray-700">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-2">
                                    <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">Amount Due</p>
                                    <p class="text-3xl font-bold text-red-600 dark:text-red-400">
                                        ${{ number_format($latestCard->amount, 2) }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-2">Due Date</p>
                    ...
                    @else
                        ...
                        <div class="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-8 text-center border border-gray-200 dark:border-gray-700">
                            <svg class="w-12 h-12 mx-auto text-gray-400 dark:text-gray-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <p class="text-gray-700 dark:text-gray-300 font-semibold">Card Payments Completed</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div>
            <div class="p-2 shadow-md border border-gray-300 dark:border-gray-700 shadow-md hover:shadow-lg transition-shadow">
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">Service Payments</h2>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Manage and track all service charges</p>
                        </div>
                        <flux:badge size="lg" variant="info">
                            {{ $patient->servicePayments->count() }} Service{{ $patient->servicePayments->count() !== 1 ? 's' : '' }}
                        </flux:badge>
                    </div>
                            ...
                            <table class="w-full">
                                <thead>
                                    <tr class="border-b border-gray-200 dark:border-gray-700">
                                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Service</th>
                                        <th class="px-6 py-4 text-right text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Amount</th>
                                        ...
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                                            <td class="px-6 py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <p class="font-medium text-gray-900 dark:text-white">{{ $servicePayment->service->name }}</p>
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                @if(!$servicePayment->is_paid)
                                                    <button
                                                        wire:click="payService({{ $servicePayment->id }})"
                        ...
                        @if($patient->servicePayments->where('is_paid', false)->count())
                            <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                                ...
                                >
                                    ...
                                    Pay All Pending Services ({{ $patient->servicePayments->where('is_paid', false)->count() }})
                                </button>
                            </div>
                        @endif
                    @else
                        ...
                        <div class="text-center py-16">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-lg font-semibold">No services registered</p>
                            <p class="text-gray-400 dark:text-gray-500 text-sm mt-2">Services will appear here once they are added</p>
                        </div>
                    @endif
                </div>
            </div>
