<div class="space-y-6" x-data="{ 
    ...
    <div
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center gap-3">
                <a href="{{ route('doctor.rehab.queue') }}" wire:navigate
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <svg class="w-7 h-7 text-purple-600 dark:text-purple-500" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="flex items-center gap-3">
                ...
                @endphp
                <span
                    class="px-4 py-2 rounded-full text-sm font-medium {{ $statusColors[$rehabEncounter->status] ?? 'bg-gray-100 text-gray-800' }}">
                    {{ $rehabEncounter->status === 'submitted_to_doctor' ? 'Pending Review' : 'Reviewed' }}
                </span>

                <!-- Action Buttons -->
                @if(in_array($rehabEncounter->status, ['submitted_to_doctor', 'doctor_review']))
                <!-- Mark as Reviewed Button -->
                <button wire:click="markAsReviewed" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading wire:target="markAsReviewed" class="w-5 h-5 animate-spin" fill="none"
                        viewBox="0 0 24 24">
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
                <span
                    class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Patient</span>
            ...
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">MRN /
                    Encounter</span>
            ...
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Submitted
                    By</span>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                    {{ $rehabStaff->name ?? 'N/A' }}
                </p>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                    {{ $rehabEncounter->updated_at->diffForHumans() }}
                </p>
            </div>
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Time
                    Elapsed</span>
                <p class="mt-1 text-lg font-semibold text-gray-900 dark:text-white">
                    {{-- {{ $rehabEncounter->$timeElapsed }} --}}
                </p>
                <p class="text-xs text-gray-600 dark:text-gray-400">
                    Since submission
                </p>
        ...
        <div class="mt-6 flex items-center justify-between">
            <div class="flex items-center flex-1">
                ...
                <div class="flex items-center relative">
                    <div
                        class="w-8 h-8 {{ $rehabEncounter->status === 'doctor_review' ? 'bg-green-500' : 'bg-indigo-500' }} rounded-full flex items-center justify-center text-white font-semibold text-sm">
                        2</div>
                    <div class="ml-2">
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Doctor Review</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $rehabEncounter->status ===
                            'doctor_review' ? 'Completed' : 'In Progress' }}</p>
                    </div>
                </div>
                <div
                    class="flex-1 mx-4 h-0.5 {{ $rehabEncounter->status === 'doctor_review' ? 'bg-green-500' : 'bg-gray-300 dark:bg-gray-600' }}">
                </div>

                <!-- Step 3: Order Packages -->
                <div class="flex items-center relative">
                    <div
                        class="w-8 h-8 {{ $rehabEncounter->status === 'doctor_review' ? 'bg-indigo-500' : 'bg-gray-300 dark:bg-gray-600' }} rounded-full flex items-center justify-center text-white font-semibold text-sm">
                        3</div>
                    <div class="ml-2">
                ...
                <div class="flex items-center relative">
                    <div
                        class="w-8 h-8 bg-purple-500 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                        4</div>
                    <div class="ml-2">
        ...
        <div
            ...
            <div class="flex items-center gap-2">
                ...
                <p class="text-sm text-purple-700 dark:text-purple-300">
                    ...
                    <span class="font-semibold">If order contains bed items → Sent to Bed Manager for bed
                        selection</span><br>
    ...
    <div class="border-b border-gray-200 dark:border-gray-700">
        <nav class="flex space-x-8">
            ...
            <button @click="activeTab = 'notes'"
                ...
                <span
                    class="ml-2 px-2 py-0.5 text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 rounded-full">
                    Added
                </span>
                @endif
            </button>
    ...
    <div x-show="activeTab === 'questionnaire'" x-cloak>
        <div
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex flex-wrap items-center gap-2 mb-2">
                                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Clinical Assessment
                                    Notes</h3>
                                <span
            ...
            <div class="{{ !$loop->first ? 'border-t border-gray-200 dark:border-gray-700' : '' }}">
                <div
                    class="px-6 py-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20">
                    <div class="flex items-center gap-2">
                        ...
                        <h4 class="font-semibold text-gray-900 dark:text-white">{{ $templateTitle }}</h4>
                ...
                <div class="p-6">
                    <div class="space-y-6">
                        ...
                        <div
                            class="group {{ !$loop->last ? 'border-b border-gray-100 dark:border-gray-700 pb-6' : '' }}">
                                ...
                                <div
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex items-center gap-2.5 text-sm">
                                            <svg class="w-4 h-4 text-purple-600 dark:text-purple-400 flex-shrink-0"
                                        ...
                                        @empty
                                        <p class="text-gray-500 dark:text-gray-400 italic text-sm">No options selected
                                        </p>
                                        @endforelse
                                    </div>
                                    @break
                                    ...
                                    @default
                                    <p
                                        class="text-gray-900 dark:text-white whitespace-pre-wrap text-sm leading-relaxed">
                                        {{ $answer->answer }}
                                    </p>
                                    @endswitch
                                </div>
                            ...
                            <div class="mt-3">
                                <div
                                    class="flex items-start gap-2.5 p-3 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-800/30">
                                    ...
                                    <div class="flex-1">
                                        <span
                                            class="text-xs font-semibold text-amber-800 dark:text-amber-400 uppercase tracking-wider">Clinical
                                            Commentary</span>
                                        <p class="mt-0.5 text-sm text-amber-700 dark:text-amber-300">{{ $answer->note }}
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="flex flex-col items-center justify-center py-16 px-4 text-center">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No questionnaire responses available
                </h3>
                <p class="text-gray-500 dark:text-gray-400 max-w-md">The rehabilitation assessment has not been
                    completed. Please check back once the evaluation is submitted.</p>
            </div>
            @endforelse
        </div>
    </div>
        ...
        <div
            ...
            <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Physician Review Notes</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400">Document your clinical assessment,
                            recommendations, and treatment plan</p>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div class="mb-6">
                    <div class="flex items-center gap-2 mb-3">
                        <div class="flex items-center gap-1.5 px-2 py-1 bg-purple-100 dark:bg-purple-900/30 rounded-md">
                            <svg class="w-3.5 h-3.5 text-purple-600 dark:text-purple-400" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 13l4 4L19 7"></path>
                            </svg>
                            <span class="text-xs font-medium text-purple-800 dark:text-purple-400">Previous
                                Assessment</span>
                        </div>
                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $rehabEncounter->updated_at->format('M
                            j, Y · g:i A') }}</span>
                    </div>
                    <div class="prose prose-sm max-w-none">
                        <div
                            class="bg-purple-50 dark:bg-purple-900/10 rounded-lg p-4 border border-purple-100 dark:border-purple-800/30">
                            <p class="text-gray-700 dark:text-gray-300 leading-relaxed">{{
                                nl2br(e($rehabEncounter->doctor_notes)) }}</p>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea id="physicianNotes" wire:model="doctorNotes" rows="6"
                            ...
                            placeholder="Document your assessment, interpretation of findings, and recommended treatment plan..."></textarea>
                        <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                            Include specific recommendations, follow-up interval, and any modifications to treatment
                            plan.
                        </p>
                    </div>

                    <div class="flex justify-end">
                        <button wire:click="saveDoctorNotes"
                            class="inline-flex items-center px-5 py-2.5 bg-purple-600 hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600 text-white font-medium rounded-lg shadow-sm hover:shadow transition-all duration-200 focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                ...
                <div
                    class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-800/30">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <span
                                class="text-xs font-semibold text-blue-800 dark:text-blue-400 uppercase tracking-wider">Documentation
                                Standards</span>
                            <ul class="mt-2 text-sm text-blue-700 dark:text-blue-300 space-y-1.5">
                                <li class="flex items-start gap-2"><span class="select-none">•</span><span>Verify
                                        completeness of assessment findings and objective measurements</span></li>
                                <li class="flex items-start gap-2"><span class="select-none">•</span><span>Document
                                        clinical reasoning and medical necessity</span></li>
                                <li class="flex items-start gap-2"><span class="select-none">•</span><span>Specify
                                        functional goals, timeframe, and follow-up plan</span></li>
                                <li class="flex items-start gap-2"><span class="select-none">•</span><span>Note any
                                        activity restrictions, precautions, or modifications</span></li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-md w-full p-6">
            ...
            <div class="flex justify-end gap-3">
                <button wire:click="$set('showConfirmModal', false)"
                    class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium transition-colors">Cancel</button>
                <button wire:click="confirmReview" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading wire:target="confirmReview" class="w-5 h-5 animate-spin" fill="none"
                        viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                        </circle>
                        <path class="opacity-75" fill="currentColor"
                    ...
                    <span wire:loading.remove wire:target="confirmReview">{{ $rehabEncounter->status === 'doctor_review'
                        ? 'Update Review' : 'Confirm Review' }}</span>
    ...
    <div class="sticky bottom-6 z-10">
        <div
            class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-4 backdrop-blur-xl bg-white/80 dark:bg-gray-800/80">
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-3">
                    ...
                    <button wire:click="proceedToOrder"
                        ...
                        <span>Proceed to Order Packages</span>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
