    ...
    <div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-6 animate-fade-in">
                <div
                    class="flex items-center p-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl shadow-lg">
                    ...
                    <div class="border-b border-gray-200 dark:border-gray-700">
                        <nav class="flex overflow-x-auto -mb-px">
                            ...
                            <button type="button"
                                ...
                                {{ $system }}
                                <span
                                    class="ml-2 px-2 py-1 text-xs rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                                    {{ count($templates) }}
                                </span>
                            </button>
                            @endforeach
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3">
                                <div
                                    ...
                                    @switch($system)
                                    @case('General')<svg class="w-5 h-5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z">
                                        </path>
                                    </svg>@break
                                    @case('Vitals')<svg class="w-5 h-5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                    </svg>@break
                                    @case('Respiratory')<svg class="w-5 h-5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4 4 0 003 15z">
                                        </path>
                                    </svg>@break
                                    @case('Abdomen')<svg class="w-5 h-5" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div>
                                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $system }}
                                        Examination</h2>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ count($templates) }}
                                        parameters to assess</p>
                                </div>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-3 gap-2">
                                    <label class="cursor-pointer">
                                        <input type="radio" wire:model="yesNoValues.{{ $template->id }}" value="yes"
                                            class="sr-only peer">
                                        <div
                                            class="w-full p-3 text-center rounded-lg border-2 border-gray-200 dark:border-gray-700 peer-checked:border-green-500 peer-checked:bg-green-50 dark:peer-checked:bg-green-900/20 transition-all duration-200">
                                            <span class="text-green-600 dark:text-green-400 font-medium">Yes</span>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" wire:model="yesNoValues.{{ $template->id }}" value="no"
                                            class="sr-only peer">
                                        <div
                                            class="w-full p-3 text-center rounded-lg border-2 border-gray-200 dark:border-gray-700 peer-checked:border-red-500 peer-checked:bg-red-50 dark:peer-checked:bg-red-900/20 transition-all duration-200">
                                            <span class="text-red-600 dark:text-red-400 font-medium">No</span>
                                        </div>
                                    </label>
                                    <label class="cursor-pointer">
                                        <input type="radio" wire:model="yesNoValues.{{ $template->id }}" value=""
                                            class="sr-only peer">
                                        <div
                                            class="w-full p-3 text-center rounded-lg border-2 border-gray-200 dark:border-gray-700 peer-checked:border-gray-400 peer-checked:bg-gray-100 dark:peer-checked:bg-gray-700 transition-all duration-200">
                                ...
                                <select wire:model="selectOptions.{{ $template->id }}"
                                    ...
                                    @php
                                    $options = is_string($template->options) ? json_decode($template->options, true) :
                                    $template->options;
                                ...
                                @case('number')
                                <div class="relative">
                                    <input type="number" step="0.1" wire:model="textValues.{{ $template->id }}"
                                        class="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-white transition-colors duration-200"
                                ...
                                @case('text')
                                <textarea wire:model="textValues.{{ $template->id }}" rows="2"
                                    class="w-full px-4 py-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:text-white transition-colors duration-200"
                ...
                <div class="sticky bottom-6 z-10">
                    <div
                        ...
                        <div class="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                            <div class="flex items-center space-x-3">
                                <button type="button" wire:click="backToChiefComplaint"
