<div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-6">
                ...
                <div class="flex items-center space-x-4">
                    <div class="bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800 rounded-xl px-4 py-3">
                        <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $summaryStats['total'] }}</div>
                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl px-4 py-3">
                        <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $summaryStats['today'] }}</div>
                    ...
                    <div class="bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 rounded-xl px-4 py-3">
                        <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ $summaryStats['last_week'] }}</div>
                ...
                @else
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="text-gray-500 dark:text-gray-400 mb-4">
                            {{ $search || $selectedType || $dateRange !== 'all' ? 'No reports match your filters' : 'Completed imaging reports will appear here' }}
                        </p>
    ...
    @if($showModal && $selectedResult)
        <div x-data="{ activeTab: 'report', zoomLevel: 1 }" 
             x-show="true"
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        {{ $selectedResult->order_details->imagingType->name }} • {{ $selectedResult->order_details->bodyPart->name }}
                                    </p>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                    ...
                    <div class="p-6 max-h-[60vh] overflow-y-auto">
                        ...
                        <div x-show="activeTab === 'report'" class="space-y-6">
                            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
                                <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Radiologist's Report</h4>
                                <div class="prose prose-green dark:prose-invert max-w-none" :style="`font-size: ${zoomLevel}em`">
                        ...
                        <div x-show="activeTab === 'details'" class="space-y-6">
                            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-4">
                                        <div>
                                            <h5 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Order Information</h5>
                                            <div class="space-y-2">
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Order ID:</span>
                                                    <span class="font-medium text-gray-900 dark:text-white">#{{ $selectedResult->order_details->id }}</span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Imaging Type:</span>
                                                    <span class="font-medium text-gray-900 dark:text-white">{{ $selectedResult->order_details->imagingType->name }}</span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Body Part:</span>
                                                    <span class="font-medium text-gray-900 dark:text-white">{{ $selectedResult->order_details->bodyPart->name }}</span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Amount:</span>
                                                    <span class="font-medium text-green-600 dark:text-green-400">${{ number_format($selectedResult->order_details->amount, 2) }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="space-y-4">
                                        <div>
                                            <h5 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Reporting Details</h5>
                                            <div class="space-y-2">
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Radiologist:</span>
                                                    <span class="font-medium text-gray-900 dark:text-white">{{ $selectedResult->radiologist->name ?? 'N/A' }}</span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Reported On:</span>
                                                    <span class="font-medium text-gray-900 dark:text-white">{{ $selectedResult->reported_at->format('M d, Y h:i A') }}</span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Images:</span>
                                                    <span class="font-medium text-gray-900 dark:text-white">{{ count($selectedResult->images ?? []) }}</span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span class="text-gray-600 dark:text-gray-400">Report Status:</span>
                                                    <span class="font-medium text-green-600 dark:text-green-400 capitalize">{{ str_replace('_', ' ', $selectedResult->status) }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
