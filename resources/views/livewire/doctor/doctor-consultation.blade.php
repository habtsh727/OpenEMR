    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div class="flex gap-4">
            ...
            <div class="bg-yellow-50 dark:bg-yellow-900/20 px-4 py-3 rounded-lg border border-yellow-100 dark:border-yellow-800">
                ...
                <div class="text-sm text-yellow-500 dark:text-yellow-300">In Progress</div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead class="bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-700 dark:to-slate-600 border-b border-gray-200 dark:border-slate-600">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">No</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Patient Info</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Vitals</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Priority</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Status</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Time</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Actions</th>
                    </tr>
                </thead>
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-100 dark:divide-slate-700">
                    @forelse ($encounters as $encounter)
                        ...
                        <tr class="hover:bg-blue-50 dark:hover:bg-slate-700/50">
                            <td class="px-6 py-4 text-sm font-semibold">{{ $loop->iteration + ($encounters->currentPage() - 1) * $encounters->perPage() }}</td>
                            <td class="px-6 py-4">
                                <div>
                                    <div class="font-medium">{{ $patient->name }}</div>
                                    <div class="text-sm text-gray-500">
                                        {{ $patient->card_number }} • {{ $patient->age }}y • {{ $patient->gender }}
                                    </div>
                                    <div class="text-sm text-gray-500">📞 {{ $patient->phone_number1 }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="space-y-1">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm">{{ $encounter->bp_systolic }}/{{ $encounter->bp_diastolic }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-xs text-gray-500">Temp:</span>
                                        <span class="text-sm">{{ $encounter->temperature }}°C</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-xs text-gray-500">Pulse:</span>
                                        <span class="text-sm">{{ $encounter->pulse }} bpm</span>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex flex-col">
                                    <span>{{ $encounter->updated_at->format('h:i A') }}</span>
                                    <span class="text-xs text-gray-500">{{ $waitingTime }} min ago</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($encounter->status === 'doctor_assigned')
