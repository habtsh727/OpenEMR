    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-6">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Imaging Types Management</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">Manage all imaging procedures and their pricing</p>
                </div>
                
                {{-- Quick Stats --}}
                <div class="flex items-center space-x-4">
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl px-4 py-3">
                        <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $types->total() }}</div>
                        <div class="text-sm font-medium text-blue-500 dark:text-blue-300">Total Types</div>
                    </div>
                    @php
                        $activeCount = $types->where('is_active', true)->count();
                        $totalFee = $types->sum('fee');
                    @endphp
                    ...
                    <div class="bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 rounded-xl px-4 py-3">
                        <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">${{ number_format($totalFee, 0) }}</div>
                        <div class="text-sm font-medium text-purple-500 dark:text-purple-300">Total Fees</div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            ...
                            <input type="text" wire:model.live.debounce.300ms="search" 
                                   placeholder="Search imaging types by name or description..." 
                        ...
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
                                    {{ $editingId ? '✏️ Edit Imaging Type' : '➕ Add New Imaging Type' }}
                                </h2>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    {{ $editingId ? 'Update the imaging procedure details' : 'Add a new imaging procedure for ordering' }}
                                </p>
                            </div>
                        ...
                        <form wire:submit.prevent="save" class="space-y-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Imaging Type Name <span class="text-red-500">*</span>
                                </label>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" wire:model="fee" 
                                           class="pl-8 w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all duration-200"
                                           step="0.01"
                                           min="0"
                                           max="999999.99"
                                           placeholder="0.00">
                                ...
                                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                    Standard fee for this imaging procedure
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea wire:model="description" 
                                          class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white transition-all duration-200"
                                          rows="3"
                                          placeholder="Add details about this imaging procedure..."></textarea>
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                <div>
                                    <h3 class="font-medium text-gray-900 dark:text-white">Status</h3>
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Active types are visible to doctors during ordering
                                    </p>
                                </div>
                            ...
                            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <button type="submit" 
                                    ...
                                    <span>{{ $editingId ? 'Update Type' : 'Add Type' }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Imaging Types List</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {{ $types->total() }} imaging type(s) found • {{ $activeCount }} active
                </p>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th wire:click="sortBy('fee')" class="px-6 py-4 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider cursor-pointer group">
                                <div class="flex items-center">
                                    <span>Fee</span>
                                    @if($sortField === 'fee')
                                        <svg class="w-4 h-4 ml-1 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($types as $type)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150 group {{ $type->id == $editingId ? 'bg-blue-50 dark:bg-blue-900/20' : '' }}">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            <div class="text-sm text-gray-500 dark:text-gray-400">
                                                Created: {{ $type->created_at->format('M d, Y') }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="toggleStatus({{ $type->id }})" 
                                        ...
                                        <span class="w-2 h-2 rounded-full mr-1.5 {{ $type->is_active ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button onclick="if(confirm('Are you sure you want to delete this imaging type?')) { @this.delete({{ $type->id }}) }" 
                ...
                <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700 dark:text-gray-300">
                            Showing <span class="font-medium">{{ $types->firstItem() }}</span> to 
                            <span class="font-medium">{{ $types->lastItem() }}</span> of 
