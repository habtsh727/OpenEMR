<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {{-- Header --}}
        <div class="mb-8">
            <div class="bg-gradient-to-r from-teal-600 to-cyan-600 rounded-2xl shadow-xl p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-2xl md:text-3xl font-bold text-white">Cupping Locations Management</h1>
                            <p class="text-teal-100 mt-1">Manage cupping therapy body locations</p>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full">
                <div class="border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-between items-center">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">
                        {{ $editingId ? 'Edit Cupping Location' : 'Add New Cupping Location' }}
                    </h3>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Location Name <span class="text-red-500">*</span></label>
                        <input type="text" wire:model="name" placeholder="e.g., Back, Shoulders, Legs, Arms, Abdomen"
                            class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:border-gray-600">
                        @error('name') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                        <textarea wire:model="description" rows="3" placeholder="Describe this treatment location..."
                            class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-teal-500 dark:bg-gray-700 dark:border-gray-600"></textarea>
                    </div>

                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" wire:model="status" class="w-4 h-4 text-teal-600 rounded">
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-end gap-3">
                    ...
                    <button wire:click="save" class="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium">Save</button>
                </div>
            </div>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($locations as $location)
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow">
                    <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-start">
                        <div>
                            <h3 class="font-bold text-gray-900 dark:text-white">{{ $location->name }}</h3>
                            @if($location->description)
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $location->description }}</p>
                            @endif
                        </div>
                        <span class="px-2 py-1 text-xs rounded-full {{ $location->status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ $location->status ? 'Active' : 'Inactive' }}
                        </span>
                    </div>
                    <div class="p-4 flex justify-end gap-2">
                        <button wire:click="toggleStatus({{ $location->id }})"
                            class="px-3 py-1.5 text-sm rounded-lg {{ $location->status ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }}">
                        ...
                        <button wire:click="delete({{ $location->id }})"
                            onclick="return confirm('Delete this cupping location?')"
            ...
            @empty
                <div class="col-span-full text-center py-12">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 font-medium">No cupping locations found</p>
                    <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Click "Add New Location" to create one</p>
                </div>
            @endforelse
        </div>
