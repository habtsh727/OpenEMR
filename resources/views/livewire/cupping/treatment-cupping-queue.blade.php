        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <select wire:model.live="dateFilter" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($sessions as $session)
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow">
                    {{-- Card Header --}}
                    <div class="bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-900/20 dark:to-teal-900/20 px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                        <div class="flex justify-between items-start">
                            ...
                            <div class="text-right">
                                @if($session->queue_position)
                                    <span class="inline-block px-2 py-1 bg-orange-100 text-orange-700 rounded-lg text-xs font-semibold">
                    ...
                    <div class="p-5 space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm">{{ \Carbon\Carbon::parse($session->session_date)->format('M d, Y') }}</span>
                        </div>

                        @if($session->treatment_started_at)
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">Started At:</span>
                            <span class="text-sm">{{ \Carbon\Carbon::parse($session->treatment_started_at)->format('h:i A') }}</span>
                        </div>
                        @endif
                        ...
                        @if($totalMaterials > 0 && $session->treatment_status !== 'completed')
                            <div class="border-t pt-2">
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <span class="font-medium {{ $materialsCollected == $totalMaterials ? 'text-green-600' : 'text-yellow-600' }}">
                                        {{ $materialsCollected }}/{{ $totalMaterials }}
                                    </span>
                                </div>
                                @if($materialsCollected < $totalMaterials)
                                    <div class="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                                        <div class="bg-yellow-500 h-1.5 rounded-full" style="width: {{ ($materialsCollected / $totalMaterials) * 100 }}%"></div>
                                    </div>
                                @endif
                            </div>
                        @endif
