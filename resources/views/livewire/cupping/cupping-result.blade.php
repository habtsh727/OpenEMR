<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($therapies as $therapy)
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-md transition-shadow">
                    <div class="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="p-5 space-y-3">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-medium text-green-600">{{ $completedSessions }}/{{ $totalSessions }}</span>
                        </div>
                        <div class="flex justify-between items-center">
            ...
            @empty
                <div class="col-span-full text-center py-12">
                    ...
                    <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Complete a treatment session to see results here</p>
                </div>
            @endforelse
        </div>
        ...
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"
             x-data="{ open: true, activeSession: 0 }"
             x-show="open"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto"
                ...
                <div class="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-sm text-gray-500">{{ $therapyDetails['patient']->name ?? 'N/A' }} (Card: {{ $therapyDetails['patient']->card_number ?? 'N/A' }})</p>
                    </div>
                    ...
                    @foreach($therapyDetails['sessions'] as $index => $sessionData)
                        <div x-show="activeSession == {{ $index }}" x-cloak class="space-y-6">
                            ...
                            <div class="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4">
                                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <p class="font-semibold">{{ \Carbon\Carbon::parse($sessionData['session']->session_date)->format('F d, Y') }}</p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="font-semibold capitalize">{{ $sessionData['session']->treatment_status }}</p>
                                    </div>
                                    @if($sessionData['session']->treatment_completed_at)
                                    <div>
                                        <p class="text-xs text-gray-500">Completed At</p>
                                        <p class="font-semibold">{{ \Carbon\Carbon::parse($sessionData['session']->treatment_completed_at)->format('M d, Y h:i A') }}</p>
                                    </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="border rounded-lg overflow-hidden">
                                <div class="bg-gray-100 dark:bg-gray-700 px-4 py-2 font-semibold">Treatments Performed ({{ count($sessionData['treatments']) }})</div>
                                <div class="p-4">
                                        ...
                                        <div class="space-y-2">
                                            @foreach($sessionData['treatments'] as $treatment)
                                                <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                                                    ...
                                                    <span class="text-sm font-semibold text-purple-600">ETB {{ number_format($treatment['price'] ?? 0, 2) }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                                        ...
                                        <div class="space-y-2">
                                            @foreach($sessionData['materials_consumed'] as $material)
                                                <div class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700/30 rounded-lg">
                                                    ...
                                                    <span class="text-sm">Quantity: {{ $material['collected_quantity'] ?? $material['quantity'] ?? 0 }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @else
                            ...
                            <div class="border rounded-lg overflow-hidden">
                                ...
                                <div class="p-4 space-y-3">
                                    @if($sessionData['report'])
                                        <div>
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Treatment Notes</p>
                                            <p class="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg">{{ $sessionData['report']->report_text ?? 'No notes' }}</p>
                                        </div>
                                        @if($sessionData['report']->observations)
                                        <div>
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Observations</p>
                                            <p class="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg">{{ $sessionData['report']->observations }}</p>
                                        </div>
                                        @endif
                                        @if($sessionData['report']->recommendations)
                                        <div>
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Recommendations</p>
                                            <p class="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg">{{ $sessionData['report']->recommendations }}</p>
                                        </div>
                                        @endif
                                        <div class="text-xs text-gray-400 mt-2">
                                            Reported by: {{ $sessionData['report']->createdBy->name ?? 'N/A' }} on {{ \Carbon\Carbon::parse($sessionData['report']->created_at)->format('M d, Y h:i A') }}
                                        </div>
                                    @else
                ...
                <div class="sticky bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 px-6 py-4 flex justify-end">
                    <button wire:click="closeModal" class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 transition">
                        Close
                    </button>
                </div>
            </div>
