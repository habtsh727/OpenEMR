                            ...
                            @forelse($packages as $package)
                                <div class="border-2 rounded-xl p-4 cursor-pointer transition-all hover:shadow-md
                                    ...
                                    <div class="flex justify-between items-start mb-2">
                                        ...
                                        <span class="text-lg font-bold text-emerald-600">ETB {{ number_format($package->total_price, 2) }}</span>
                                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20">
                        <div class="flex justify-between items-center">
                            ...
                            <button wire:click="$set('showPackagePreview', false)" class="text-gray-400 hover:text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="p-6 space-y-4">
                                ...
                                @foreach($packageTreatments as $treatment)
                                    <div class="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                        <div>
                                            ...
                                            <span class="text-sm text-gray-500 ml-2">on {{ $treatment['location_name'] }}</span>
                                        </div>
                                        <span class="text-emerald-600 font-semibold">ETB {{ number_format($treatment['price'], 2) }}</span>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <!-- Materials -->
                        <div>
                            <h3 class="font-semibold text-gray-900 dark:text-white mb-2">Materials Required:</h3>
                            <div class="space-y-2">
                        ...
                        <div class="border-t pt-3">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-2xl font-bold text-emerald-600">ETB {{ number_format($packageTotalPrice, 2) }}</span>
                            </div>
                        </div>
                        ...
                        <div class="border-t pt-4 mt-2">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Number of Sessions</label>
                                    <div class="flex items-center gap-2">
                                        <button wire:click="$set('totalSessions', max(1, {{ $totalSessions }} - 1))"
                                            class="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200">-</button>
                                        <input type="number" wire:model="totalSessions" min="1" max="30"
                                            class="w-20 text-center px-2 py-1 border rounded-lg dark:bg-gray-700 dark:border-gray-600">
                                        <button wire:click="$set('totalSessions', {{ $totalSessions }} + 1)"
                                            class="w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700 hover:bg-gray-200">+</button>
                                    </div>
                                </div>
                                <div class="flex items-end">
                                    <button wire:click="addToCart"
                                        class="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-2 rounded-lg font-medium transition-all">
                                        Add to Order
                                    </button>
                                </div>
                            </div>
                            <p class="text-xs text-gray-500 mt-2">
                                Total for {{ $totalSessions }} session(s): ETB {{ number_format($packageTotalPrice * $totalSessions, 2) }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden sticky top-6">
                    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900">
                        <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                            ...
                            Order Summary
                            @if(count($cart) > 0)
                                <span class="ml-2 px-2 py-0.5 bg-emerald-100 text-emerald-800 text-xs rounded-full">{{ count($cart) }} item(s)</span>
                            @endif
                        </h2>
                    </div>
                                    ...
                                    <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-3">
                                        <div class="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <div class="text-xs text-gray-500">{{ $item['sessions'] }} session(s)</div>
                                            </div>
                                        ...
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600">@ ETB {{ number_format($item['package_price'], 2) }}/session</span>
                                            <span class="font-semibold text-emerald-600">ETB {{ number_format($item['total_amount'], 2) }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full p-6">
                <div class="text-center">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 mb-4">
                        Total amount: <span class="font-bold text-emerald-600">ETB {{ number_format($cartTotal, 2) }}</span>
                        <br>Total sessions: <span class="font-bold">{{ array_sum(array_column($cart, 'sessions')) }}</span>
                    </p>
