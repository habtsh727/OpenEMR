    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div
                ...
                {{-- Card Header --}}
                <div
                    class="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="font-bold text-gray-900 dark:text-white">{{ $therapy->encounter->patient->name ??
                                'N/A' }}</h3>
                            <p class="text-xs text-gray-500 mt-1">Card: {{ $therapy->encounter->patient->card_number ??
                                'N/A' }}</p>
                        </div>
                ...
                <div class="p-5 space-y-3">
                    ...
                    <div>
                        ...
                        <div class="space-y-1">
                            ...
                            <div class="text-sm">
                                ...
                                <div class="text-xs text-gray-500">@ ETB {{
                                    number_format($group->first()->package_price_snapshot, 2) }}/session</div>
                            </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-100 dark:border-gray-700 pt-3">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-white">ETB {{ number_format($totalAmount,
                                2) }}</span>
                        </div>
                        <div class="flex justify-between items-center mb-2">
                    ...
                    <div class="mt-2">
                        @if($pendingAmount == 0)
                        <span
                            class="inline-block w-full text-center px-3 py-1.5 bg-green-100 text-green-700 rounded-lg text-sm font-medium">
                            Fully Paid ✓
                        </span>
                        @elseif($totalPaid > 0)
                        <span
                            class="inline-block w-full text-center px-3 py-1.5 bg-yellow-100 text-yellow-700 rounded-lg text-sm font-medium">
                            Partial Payment - {{ $unpaidSessions }} session(s) pending
                        </span>
                        @else
                        <span
                            class="inline-block w-full text-center px-3 py-1.5 bg-red-100 text-red-700 rounded-lg text-sm font-medium">
        ...
        <div class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" x-data="{
     ...
     }" x-init="updateSelectAll()" x-on:select-all-changed.window="updateSelectAll()">

            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">

                <div
                    class="sticky top-0 bg-white dark:bg-gray-800 border-b px-6 py-4 flex justify-between items-center">
                    <div>
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-3 max-h-96 overflow-y-auto mb-6">
                        ...
                        {{ $session['selected'] ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700' }}"
                            ...
                            <input type="checkbox" wire:model="sessionsList.{{ $index }}.selected"
                                ...
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500">{{
                                            \Carbon\Carbon::parse($session['session_date'])->format('l, F d, Y') }}
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <div class="font-semibold">ETB {{ number_format($session['amount'], 2) }}</div>
                                        @if($session['paid_amount'] > 0)
                                        <div class="text-xs text-green-600">Paid: ETB {{
                                            number_format($session['paid_amount'], 2) }}</div>
                                        <div class="text-xs text-red-600">Due: ETB {{
                                            number_format($session['remaining'], 2) }}</div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="border-t pt-4">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <span class="text-2xl font-bold text-blue-600">
                                ETB {{ number_format(array_sum(array_column(array_filter($sessionsList, function($s) {
                                return $s['selected']; }), 'remaining')), 2) }}
                            </span>
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model.live="paymentAmount" step="0.01"
                            class="w-full px-3 py-2 border rounded-lg text-lg font-semibold">
                    </div>
                ...
                <div class="sticky bottom-0 bg-white dark:bg-gray-800 border-t px-6 py-4 flex gap-3">
                    ...
                    <button wire:click="confirmPayment"
                        class="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-semibold">Confirm
                        Payment</button>
                </div>
            </div>
        </div>
