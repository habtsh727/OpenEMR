<div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-2xl shadow-xl p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
            ...
            <div class="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 {{ now()->format('H') < 17 ? 'animate-pulse' : '' }}">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <!-- Doctor Filter -->
            <div>
                <select wire:model.live="doctorId" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white">
            ...
            <!-- Visit Type Filter -->
            <div>
                <select wire:model.live="visitType" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white">
            ...
            <!-- Group By -->
            <div>
                <select wire:model.live="groupBy" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 dark:bg-gray-700 dark:text-white">
        ...
        <div class="flex flex-wrap items-center gap-2 mt-3">
            @if($search)
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
                    Search: {{ $search }}
            ...
            @if($search || $doctorId || $visitType)
                <button wire:click="resetFilters" class="text-sm text-emerald-600 hover:text-emerald-800 dark:text-emerald-400">
                    Clear All
                </button>
            @endif
        </div>
        ...
        <div class="space-y-6">
                    ...
                    <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                @if($group['is_today'])
                                    <span class="px-3 py-1 bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 rounded-full text-sm font-medium">
                                        Today
                                    </span>
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-4">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            <div class="h-10 w-10 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-medium">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @if($appointment->isCheckInAvailable())
                                            <button wire:click="checkIn({{ $appointment->id }})"
                                                    class="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded-lg transition-colors">
                                                Check In
                                            </button>
                                        @endif
            ...
            @empty
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
                    ...
                    <a href="{{ route('appointments.create') }}" class="inline-block mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
    ...
    @elseif($groupBy === 'doctor')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            @empty
                <div class="col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
            ...
            @forelse($groupedAppointments as $group)
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="px-6 py-4 bg-gradient-to-r from-purple-500 to-indigo-600">
                        ...
                        <p class="text-purple-100 text-sm mt-1">{{ $group['count'] }} appointment{{ $group['count'] != 1 ? 's' : '' }}</p>
                    </div>
                    <div class="divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($group['appointments']->take(5) as $appointment)
                            <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                            Dr. {{ $appointment->doctor->name }} • 
                                            {{ $appointment->appointment_date->format('M d') }} at {{ $appointment->appointment_time->format('h:i A') }}
                                        </div>
                                    </div>
                                    <span class="px-2 py-1 text-xs rounded-full {{ $appointment->status_color }}">
                                        {{ ucfirst($appointment->status) }}
