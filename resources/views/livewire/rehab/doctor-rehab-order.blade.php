<div class="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
    ...
    <div
        ...
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center gap-3">
                <button wire:click="backToReview"
                    ...
                    <svg class="w-5 h-5 text-gray-600 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-200"
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                        <svg class="w-7 h-7 text-indigo-600 dark:text-indigo-500" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <span class="font-semibold text-gray-900 dark:text-white">{{
                            $rehabEncounter->encounter->patient->name }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                <span
                    class="px-4 py-2 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 text-sm font-medium rounded-full">
                    {{ $rehabEncounter->status_label ?? 'Doctor Review' }}
                </span>

                @if($hasBedItems)
                <span
                    class="px-4 py-2 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-400 text-sm font-medium rounded-full flex items-center gap-1">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 font-medium text-gray-900 dark:text-white">{{
                        $rehabEncounter->encounter->patient->name }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                <div class="p-2 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
                ...
                <div>
                    <span
                        class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Encounter</span>
                    <p class="mt-1 font-medium text-gray-900 dark:text-white font-mono">#{{
                        $rehabEncounter->encounter_id }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                ...
                <div>
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Review
                        Status</span>
        ...
        <div
            class="mt-4 p-4 {{ $hasBedItems ? 'bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800' }} rounded-xl border">
            <div class="flex items-center gap-3">
                <div
                    class="p-2 {{ $hasBedItems ? 'bg-purple-200 dark:bg-purple-800' : 'bg-blue-200 dark:bg-blue-800' }} rounded-lg">
                    @if($hasBedItems)
                    <svg class="w-5 h-5 text-purple-700 dark:text-purple-300" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                    </svg>
                ...
                <div>
                    <p
                        class="text-sm font-medium {{ $hasBedItems ? 'text-purple-800 dark:text-purple-300' : 'text-blue-800 dark:text-blue-300' }}">
                        Next: <span class="font-bold">{{ $destination }}</span>
                    </p>
                    <p
                        class="text-xs {{ $hasBedItems ? 'text-purple-600 dark:text-purple-400' : 'text-blue-600 dark:text-blue-400' }} mt-0.5">
                        @if($hasBedItems)
    ...
    <div
        ...
        <div
            class="border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-900/20 dark:to-blue-900/20">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div
                    class="border rounded-xl p-5 transition-all duration-200 {{ in_array($package->id, $selectedPackageIds) ? 'bg-indigo-50/50 dark:bg-indigo-900/20 border-indigo-300 dark:border-indigo-700' : 'border-gray-200 dark:border-gray-700 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-md dark:hover:bg-gray-700/50' }}">
                    <div class="flex justify-between items-start gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-2">
                                <h3 class="font-semibold text-gray-900 dark:text-white truncate">{{ $package->name }}
                                </h3>
                                @if(in_array($package->id, $selectedPackageIds))
                                <span
                                    class="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-400 text-xs font-medium rounded-full flex items-center gap-1">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{{ $package->description }}
                            </p>
                            @endif
                            ...
                            <div class="flex items-center gap-4 mt-3">
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    @php
                                    $medCount = $package->items->whereIn('item_type', ['standard_medication',
                                    'custom_medication'])->count();
                                    $serviceCount = $package->items->where('item_type', 'service')->count();
                                    $hasBed = $package->items->where('item_type', 'bed')->isNotEmpty();
                                    @endphp
                                    @if($medCount > 0)
                                    <span
                                        class="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-400 text-xs font-medium rounded-full flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 3v9a1 1 0 01-1 1h-4a1 1 0 01-1-1V7L8 4z">
                                            </path>
                                        </svg>
                                        {{ $medCount }} Meds
                                    </span>
                                    ...
                                    @if($serviceCount > 0)
                                    <span
                                        class="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-400 text-xs font-medium rounded-full flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    @if($hasBed)
                                    <span
                                        class="px-2 py-0.5 bg-purple-100 dark:bg-purple-900/50 text-purple-800 dark:text-purple-400 text-xs font-medium rounded-full flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M3 12l2-2 2 2 2-2 2 2 2-2 2 2 2-2 2 2 2-2 2 2M5 12v6M3 20h18M15 12v6">
                                            </path>
                                        </svg>
                                        Bed
                                    </span>
        ...
        <div class="p-6">
            <div class="space-y-4">
                ...
                <div
                    ...
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-2">
                                <h3 class="font-semibold text-gray-900 dark:text-white">{{ $orderPackage->package_name
                                    }}</h3>
                                <span
                                    class="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 text-xs font-medium rounded-full">
                                    Package
                                </span>
                            ...
                            <p class="text-lg font-bold text-indigo-600 dark:text-indigo-500">ETB {{
                                number_format($orderPackage->final_price, 2) }}</p>
                        </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                        <h4
                            class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            ...
                            <div
                                class="text-sm p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-100 dark:border-gray-600">
                                <div class="flex justify-between items-center">
                                    <span class="font-medium text-gray-700 dark:text-gray-300">{{ $item->item_name
                                        }}</span>
                                    <span
            ...
            <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                <div class="flex justify-between items-center">
                    ...
                    <span class="text-2xl font-bold text-gray-900 dark:text-white">ETB {{ number_format($totalAmount, 2)
                        }}</span>
                </div>

                @if($hasBedItems)
                <p
                    class="mt-2 text-sm text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg flex items-center gap-2">
