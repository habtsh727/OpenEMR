    ...
    <form wire:submit.prevent="submitQuestionnaire" class="space-y-6">
        ...
        <div id="question-{{ $question->id }}" wire:key="question-{{ $question->id }}"
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-start gap-3 flex-1">
                    <span
                        class="flex items-center justify-center w-6 h-6 rounded-full bg-gradient-to-br from-teal-500 to-cyan-600 text-white text-xs font-bold">
                        {{ $loop->iteration }}
                    </span>
                    <div class="flex-1">
                ...
                <div class="flex items-center gap-2">
                    <span
                        class="px-3 py-1 text-xs font-medium rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">
            ...
            <div class="space-y-4">
                ...
                <div class="flex gap-6">
                    <label
                        class="flex items-center gap-2 cursor-pointer p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                ...
                <input type="number" wire:model.live="answers.{{ $question->id }}.value"
                    class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-700 dark:text-white"
                    placeholder="Enter number..." min="0" step="any">
                @break
                ...
                <!-- Add Note Button -->
                <div x-show="showNoteFor !== {{ $question->id }}" x-cloak class="flex justify-end">
                    <button type="button" @click="showNoteFor = {{ $question->id }}"
                ...
                <div x-show="showNoteFor === {{ $question->id }} || @js(!empty($answers[$question->id]['note']))"
                    x-collapse class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl">
                    <div class="flex items-start gap-2">
                        <svg class="w-5 h-5 text-yellow-600 dark:text-yellow-500 mt-1 flex-shrink-0" fill="none"
                        ...
                        <button type="button" @click="showNoteFor = null"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2 mb-4">
                <svg class="w-5 h-5 text-teal-600 dark:text-teal-500" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <textarea wire:model.live="rehabNotes" rows="3"
                class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-700 dark:text-white"
                placeholder="Add overall observations, recommendations, or notes about this rehabilitation case..."></textarea>
        </div>

        <!-- Action Buttons -->
        <div
            class="sticky bottom-6 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-6 backdrop-blur-sm bg-white/90 dark:bg-gray-800/90">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" wire:click="saveProgress" wire:loading.attr="disabled"
                        ...
                        <svg wire:loading wire:target="saveProgress" class="w-5 h-5 animate-spin" fill="none"
                            viewBox="0 0 24 24">
                ...
                <button type="submit" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading wire:target="submitQuestionnaire" class="w-5 h-5 animate-spin" fill="none"
                        viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4">
                        </circle>
                        <path class="opacity-75" fill="currentColor"
