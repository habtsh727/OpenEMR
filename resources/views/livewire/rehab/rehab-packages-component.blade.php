...
<div>
                ...
                <div class="bg-white dark:bg-gray-800 p-4 rounded-lg border dark:border-gray-700 hover:shadow-md transition-shadow">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            ...
                            <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats['total_items'] }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Final Price</th>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-wrap gap-1">
                                ...
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-{{ $item->type_color }}-100 text-{{ $item->type_color }}-800 dark:bg-{{ $item->type_color }}-900 dark:text-{{ $item->type_color }}-200">
                                    {{ $item->item_name }}
                                </span>
                                @endforeach
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                {{ $search ? 'No packages match your search' : 'Get started by creating your first rehab package' }}
                            </p>
                            @if(!$search)
                            <button wire:click="showCreateForm"
                ...
                <div class="px-6 py-4 max-h-[80vh] overflow-y-auto">
                    <form wire:submit.prevent="savePackage" class="space-y-6">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                            <h4 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Package Information</h4>
                        ...
                        <!-- Pricing -->
                        <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                            <h4 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Pricing & Discounts</h4>
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                    ...
                                    <div class="relative">
                                        <span class="absolute left-3 top-2 text-gray-500 dark:text-gray-400">ETB</span>
                                    ...
                                    @error('base_price') <p class="text-red-600 dark:text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    @error('discount_value') <p class="text-red-600 dark:text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Final Price</label>
                                    <div class="relative">
                                        <span class="absolute left-3 top-2 text-gray-500 dark:text-gray-400">ETB</span>
                                        <input type="text" value="{{ number_format($final_price, 2) }}" readonly
                        ...
                        <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="bg-white dark:bg-gray-800 p-3 rounded-lg border dark:border-gray-700 flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        <span class="w-2 h-2 rounded-full bg-{{ $item['type_color'] ?? 'gray' }}-500"></span>
                                        <div>
                                            <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $item['item_name'] }}</span>
                                            <span class="text-xs text-gray-500 dark:text-gray-400 ml-2">{{ $item['type_label'] ?? $item['item_type'] }}</span>
                                            @if(isset($item['display_name']) && $item['display_name'] !== $item['item_name'])
                                            <span class="text-xs text-gray-500 dark:text-gray-400 block">{{ $item['display_name'] }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="flex items-center space-x-2">
                            ...
                            <div class="text-center py-8 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg">
                                ...
                                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No items added</h3>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Click "Add Item" to add items to this package.</p>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="flex justify-end space-x-3 pt-4 border-t dark:border-gray-700">
                            ...
                            <button type="submit" wire:loading.attr="disabled"
                                class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-700 hover:from-blue-700 hover:to-indigo-800 text-white rounded-lg font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all duration-200 flex items-center">
                                <span wire:loading.remove>{{ $editingId ? 'Update' : 'Create' }} Package</span>
    ...
    <!-- Item Form Modal -->
    @if($showItemForm)
    <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="item-modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 transition-opacity"
                aria-hidden="true"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div
                ...
                <div class="px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-700 dark:from-purple-700 dark:to-indigo-800">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center space-x-4">
                            <div class="p-2 bg-white/20 rounded-lg backdrop-blur-sm">
                                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-white">{{ $itemFormTitle }}</h3>
                                <p class="text-sm text-white/90">Configure package item details</p>
                            </div>
                        </div>
                ...
                <div class="px-6 py-4">
                    <form wire:submit.prevent="saveItem" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select wire:model="item_type"
                                    class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                                    <option value="">Select Type</option>
                                    @foreach($itemTypes as $value => $label)
                                    <option value="{{ $value }}">{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('item_type') <p class="text-red-600 dark:text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div>
                                ...
                                @error('item_name') <p class="text-red-600 dark:text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                            </div>
                            ...
                            @if($item_type === 'bed')
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bed Duration (Days)</label>
                        ...
                        <div class="flex justify-end space-x-3 pt-4 border-t dark:border-gray-700">
                            ...
                            <button type="submit"
                                class="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-700 hover:to-indigo-800 text-white rounded-lg font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-all duration-200">
