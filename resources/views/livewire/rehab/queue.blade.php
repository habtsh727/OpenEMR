    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                @php
                    $statusConfig = [
                        'pending_questionnaire' => ['label' => 'Pending', 'bg' => 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400', 'border' => 'border-yellow-200'],
                        'questionnaire_in_progress' => ['label' => 'In Progress', 'bg' => 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', 'border' => 'border-blue-200'],
                        'submitted_to_doctor' => ['label' => 'Submitted', 'bg' => 'bg-purple-50 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', 'border' => 'border-purple-200'],
                        'doctor_review' => ['label' => 'Reviewed', 'bg' => 'bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400', 'border' => 'border-green-200'],
                    ];
                @endphp
                
                @foreach($statusConfig as $status => $config)
                    @php $count = $statusCounts[$status] ?? 0; @endphp
                    <div class="px-4 py-2 rounded-lg border {{ $config['bg'] }} {{ $config['border'] }} flex items-center gap-2">
                        ...
                        <tr wire:key="{{ $rehab->id }}" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-full flex items-center justify-center text-white font-semibold">
                                        {{ strtoupper(substr($rehab->encounter->patient->first_name ?? '?', 0, 1) . substr($rehab->encounter->patient->last_name ?? '?', 0, 1)) }}
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">
                                @if($rehab->filledBy)
                                    <div class="flex items-center">
                                        <div class="w-6 h-6 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center text-xs font-medium text-gray-700 dark:text-gray-300">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                @if($rehab->status === 'pending_questionnaire')
                                    <button class="inline-flex items-center px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium rounded-lg transition-colors opacity-50 cursor-not-allowed" disabled>
                                        Start Questionnaire
                                    </button>
