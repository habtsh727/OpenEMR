    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Assessment Templates</h1>
                    <p class="mt-2 text-gray-600 dark:text-gray-400">Manage diagnosis templates used for patient assessments</p>
                </div>
                ...
                <div class="flex items-center space-x-4">
                    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl px-4 py-3">
                        ...
                        <div class="text-sm font-medium text-blue-500 dark:text-blue-300">Total Diagnoses</div>
                    </div>
                    @php
                        $activeCount = $templates->where('active', true)->count();
                        $contextCount = $templates->unique('context')->count();
                    @endphp
                    ...
                    <div class="bg-purple-50 dark:bg-purple-900/20 border border-purple-100 dark:border-purple-800 rounded-xl px-4 py-3">
                        <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ $contextCount }}</div>
                        <div class="text-sm font-medium text-purple-500 dark:text-purple-300">Contexts</div>
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6">
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    {{-- Left Side: Search and Context Filter --}}
                    <div class="flex-1">
                        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            <div class="flex items-center space-x-2">
                                <span class="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">Context:</span>
                                <div class="flex flex-wrap gap-1">
                                    <button wire:click="$set('contextFilter', null)" 
                                            class="px-3 py-1.5 text-sm rounded-lg transition-all duration-200 {{ !$contextFilter ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }}">
                                        All
                                    </button>
                                    @foreach($contexts ?? [] as $context)
                                    <button wire:click="$set('contextFilter', '{{ $context }}')" 
                                            class="px-3 py-1.5 text-sm rounded-lg transition-all duration-200 {{ $contextFilter === $context ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }}">
                                        {{ $context }}
                                    </button>
                                    @endforeach
                                </div>
                        ...
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
                                    {{ $isEditing ? '✏️ Edit Diagnosis' : '➕ Add New Diagnosis' }}
                                </h2>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                    {{ $isEditing ? 'Update the assessment diagnosis template' : 'Add a new diagnosis for patient assessments' }}
                                </p>
                            </div>
                        ...
                        <form wire:submit.prevent="{{ $isEditing ? 'update' : 'create' }}" class="space-y-6">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                    Diagnosis Name <span class="text-red-500">*</span>
                                </label>
                                <input type="text" wire:model="diagnosis" 
                                       ...
                                       placeholder="e.g., Malaria, Pneumonia, Hypertension, Diabetes Mellitus"
                                       autofocus>
                                @error('diagnosis') 
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    @foreach(['OPD', 'ER', 'IPD'] as $option)
                                    <label class="relative flex cursor-pointer">
                                        <input type="radio" wire:model="context" value="{{ $option }}" class="sr-only peer">
                                        <div class="w-full p-4 border border-gray-300 dark:border-gray-600 rounded-lg peer-checked:border-blue-500 peer-checked:bg-blue-50 dark:peer-checked:bg-blue-900/20 peer-checked:ring-2 peer-checked:ring-blue-200 dark:peer-checked:ring-blue-800 transition-all duration-200">
                                            <div class="flex items-center">
                                                ...
                                                <div>
                                                    <div class="font-medium text-gray-900 dark:text-white">{{ $option }}</div>
                                ...
                                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                                    Select the clinical context where this diagnosis is commonly used
                                </p>
                            </div>
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">
                                        Active diagnoses are visible to doctors during assessment
                                    </p>
                                </div>
                            ...
                            <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <button type="submit" 
                                    ...
                                    <span>{{ $isEditing ? 'Update Diagnosis' : 'Add Diagnosis' }}</span>
                                </button>
                            </div>
                        </form>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Diagnosis List</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {{ $templates->total() }} diagnosis(es) found across {{ $templates->unique('context')->count() }} contexts
                </p>
            </div>
                        ...
                        @forelse($templates as $template)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150 group">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                        $contextColor = match($template->context) {
                                            'OPD' => 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
                                            'ER' => 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
                                            'IPD' => 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
                                            default => 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300',
                                        };
                                        
                                        $contextLabel = match($template->context) {
                                            'OPD' => 'Outpatient',
                                            'ER' => 'Emergency',
                                            'IPD' => 'Inpatient',
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium {{ $contextColor }}">
                                        @if($template->context === 'OPD')
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                            </svg>
                                        @elseif($template->context === 'ER')
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                        @elseif($template->context === 'IPD')
                                            <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button wire:click="toggleStatus({{ $template->id }})" 
                                        ...
                                        <span class="w-2 h-2 rounded-full mr-1.5 {{ $template->active ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button onclick="if(confirm('Are you sure you want to delete this diagnosis?')) { @this.delete({{ $template->id }}) }" 
