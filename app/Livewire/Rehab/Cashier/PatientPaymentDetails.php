    ...
    try {
        DB::transaction(function () {
            ...
            if ($pendingInstallments === 0) {
                ...
                // FIRST INSTALLMENT FULLY PAID - KEEP IN CASHIER QUEUE
                $this->order->payment_status = 'partial';
                $this->order->status = 'sent_to_cashier'; // FIXED: Don't change to 'paid'
                $this->order->payment_method = $this->paymentMethod;
                
                // DON'T send to rehab yet - they need to pay more
                // Just update next payment due date
                ...
                $this->alertMessage = "First installment paid! Next payment due: " . 
                    ($nextInstallment ? $nextInstallment->due_date->format('M d, Y') : 'N/A');
                $this->alertType = 'success';
                
            } else {
                // PARTIAL PAYMENT ON ANY INSTALLMENT
                $nextInstallment = $this->order->paymentInstallments()
                    ->where('status', 'pending')
                ...
                $this->order->payment_status = 'partial';
                $this->order->status = 'sent_to_cashier'; // Keep in cashier queue
